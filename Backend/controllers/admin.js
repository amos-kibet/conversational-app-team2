...
const adminController = async (userPayload, role, res) => {
  try {
    ...
    console.log("[ADMIN_REGISTER_CONTROLLER] 1: " + Object.keys(userPayload));
    console.log("[ADMIN_REGISTER_CONTROLLER] 2: " + userPayload.username);
    if (usernameTaken) {
    ...
    console.log("[ADMIN_REGISTER_CONTROLLER] 3: " + userPayload.email);
    ...
    console.log("[ADMIN_REGISTER_CONTROLLER] 4: " + error.message);
...
const Logincontroller = async (userPayload, role, res) => {
  try {
    ...
    console.log("[ADMIN_LOGIN_CONTROLLER] 1: " + Object.keys(userPayload));
    console.log("[ADMIN_LOGIN_CONTROLLER] 2: " + userPayload.username);
    if (!user) {
    ...
    //The above logic means user is existing. And user is trying to sign in from the correct portal
    ...
    console.log("[ADMIN_LOGIN_CONTROLLER] 3: " + error.message);
    return res.status(500).json({
      msg: "Unable to login to your account. Please try again.",
      success: false,
    });
  }
};
