...
const username = require("../services/repo/validate");
const email = require("../services/repo/validate");
...
const adminController = async (userPayload, role, res) => {
  try {
    ...
    let usernameNTaken = await username.username(User, userPayload.username);
    if (usernameNTaken) {
    ...
    console.log("[ADMIN_REGISTER_CONTROLLER]: " + error.message);
    console.log(
      "[ADMIN_REGISTER_CONTROLLER_PAYLOAD]: " + Object.keys(userPayload)
    );
...
const Logincontroller = async (userPayload, role, res) => {
  try {
    ...
    console.log(
      "[ADMIN_LOGIN_CONTROLLER_PAYLOAD]: " + Object.keys(userPayload)
    );
    ...
    console.log("[ADMIN_LOGIN_CONTROLLER]: " + error.message);
    return res.status(500).json({
